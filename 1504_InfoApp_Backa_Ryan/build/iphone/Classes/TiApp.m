...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1504_InfoApp_Backa_RyanUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1504_InfoApp_Backa_RyanUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1504_InfoApp_Backa_Ryan app using the node.js CLI
