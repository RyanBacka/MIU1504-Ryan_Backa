...
NSString * k1504_InfoApp_Backa_Ryan$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1504_InfoApp_Backa_RyanObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1504_infoapp_backa_ryan);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create k1504_InfoApp_Backa_Ryan global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1504_InfoApp_Backa_Ryan" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1504_infoapp_backa_ryan = [[k1504_InfoApp_Backa_RyanObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1504_infoapp_backa_ryan];
	
	NSString *_k1504_infoapp_backa_ryanNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1504_infoapp_backa_ryanNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1504_infoapp_backa_ryan valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1504_InfoApp_Backa_Ryan$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1504_infoapp_backa_ryan.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.k1504_infoapp_backa_ryan.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
